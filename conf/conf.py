...
class SysConfigs:
    ...
    def _init_option(mcs):
        for item in mcs._get_keys():
            if not SysConfigsModel.objects.filter(key=item).exists():
                default_value = getattr(ConfigDefaultValue, item)
                if callable(default_value):
                    default_value = default_value()
                try:
                    SysConfigsModel.objects.create(key=item, value=default_value)
    ...
    def _get_option(mcs, option_key):
        try:
            option = SysConfigsModel.objects.get(key=option_key)
            value = option.value
            return value
        except SysConfigsModel.DoesNotExist:
            mcs._init_option()
    ...
    def _set_option(mcs, option_key: str, option_value):
        try:
            with transaction.atomic():
                option = SysConfigsModel.objects.select_for_update().get(key=option_key)
    ...
    def website_base_url(cls):
        return cls._get_option(ConfigKeys.website_base_url)

    @website_base_url.setter
    def website_base_url(cls, value):
        cls._set_option(ConfigKeys.website_base_url, value)

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def website_name(cls):
        return cls._get_option(ConfigKeys.website_name)

    @website_name.setter
    def website_name(cls, value):
        cls._set_option(ConfigKeys.website_name, value)

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def website_name_shortcut(cls):
        return cls._get_option(ConfigKeys.website_name_shortcut)

    @website_name_shortcut.setter
    def website_name_shortcut(cls, value):
        cls._set_option(ConfigKeys.website_name_shortcut, value)

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def website_footer(cls):
        return cls._get_option(ConfigKeys.website_footer)

    @website_footer.setter
    def website_footer(cls, value):
        cls._set_option(ConfigKeys.website_footer, value)

    @my_property
    def allow_register(cls):
        return cls._get_option(ConfigKeys.allow_register)

    @allow_register.setter
    def allow_register(cls, value):
        cls._set_option(ConfigKeys.allow_register, value)

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def submission_list_show_all(cls):
        return cls._get_option(ConfigKeys.submission_list_show_all)

    @submission_list_show_all.setter
    def submission_list_show_all(cls, value):
        cls._set_option(ConfigKeys.submission_list_show_all, value)

    @my_property
    def smtp_config(cls):
        return cls._get_option(ConfigKeys.smtp_config)

    @smtp_config.setter
    def smtp_config(cls, value):
        cls._set_option(ConfigKeys.smtp_config, value)

    @my_property
    def judge_server_token(cls):
        return cls._get_option(ConfigKeys.judge_server_token)

    @judge_server_token.setter
    def judge_server_token(cls, value):
        cls._set_option(ConfigKeys.judge_server_token, value)

    @my_property
    def throttling(cls):
        return cls._get_option(ConfigKeys.throttling)

    @throttling.setter
    def throttling(cls, value):
        cls._set_option(ConfigKeys.throttling, value)

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def languages(cls):
        return cls._get_option(ConfigKeys.languages)

    @languages.setter
    def languages(cls, value):
        cls._set_option(ConfigKeys.languages, value)
